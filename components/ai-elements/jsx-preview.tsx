...
const createStringStore = (initialValue: string): StringStore => {
...
export const JSXPreview = memo(
  ({
    ...
    const error = errorState?.jsx === jsx ? errorState.error : null;
...
interface JSXPreviewContentBodyProps extends JSXPreviewContentProps {
  bindings: JsxParserProps["bindings"];
  components: JsxParserProps["components"];
  isStreaming: boolean;
  lastGoodJsx: string;
  lastGoodStore: StringStore;
  onErrorProp?: (error: Error) => void;
  processedJsx: string;
  setError: (error: Error | null) => void;
}

const JSXPreviewContentBody = ({
  className,
  bindings,
  components,
  isStreaming,
  ...
  useEffect(() => {
    ...
    lastGoodStore.setValue(processedJsx);
  }, [lastGoodStore, processedJsx]);
...
export const JSXPreviewContent = memo(
  ({ className, ...props }: JSXPreviewContentProps) => {
    ...
    const [lastGoodStore] = useState(() => createStringStore(processedJsx));
    const lastGoodJsx = useSyncExternalStore(
      lastGoodStore.subscribe,
      lastGoodStore.getSnapshot,
      lastGoodStore.getSnapshot
    );

    return (
      <JSXPreviewContentBody
        {...props}
        bindings={bindings}
        className={className}
        components={components}
...
JSXPreviewContentBody.displayName = "JSXPreviewContentBody";
