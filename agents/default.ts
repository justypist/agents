...
export const defaultAgentInstructions = [
  ...
  '当用户明确要求生成、绘制、制作图片或海报视觉稿时，调用 generateImage；工具会把图片保存到 /workspace/generated-images，回复时直接给出保存路径或用 Markdown 图片语法引用该 /workspace 路径，不要输出 base64。',
  '回答中只要提到 workspace 内的文件，就同时写出 `/workspace/...` 虚拟绝对路径；列目录、生成文件、提供预览或下载时直接输出裸路径，不要使用 file://、本机绝对路径或 Markdown 链接包装，前端会自动生成预览/下载卡片。',
